...
# Intentionally broad sanity rails. These are not price predictions; they only keep clearly contaminated listings out of trusted totals.
PRICE_BANDS={
...
def product_type_clean(pid,t):
 ...
 # Complete systems commonly contain the exact CPU/GPU/RAM terms and were the main false-positive source.
...
def match_result(p,r,pr=None):
 ...
 exact={
  ...
  'case':(('lancool','217'),('walnut','wood','white')),
 ...
 if pid=='psu-1000w':
  return (True,'approved 1000W PSU + sane price') if '1000w' in t and any(x in t for x in ('rm1000x','a1000gl')) else (False,'not approved 1000W PSU')
 if pid=='psu-1200w':
  return (True,'approved 1200W PSU + sane price') if '1200w' in t and 'vertex' in t and 'gx' in t else (False,'not approved 1200W PSU')
 return False,'no strong rule for this item'

def revalidate_existing():
 """Re-run every historical SerpApi priced observation through current rules. No network calls."""
 ...
 for path in OBS.glob('*.json'):
  ...
  for row in rows:
   ...
   new_status='verified' if ok else 'manual_review'; new_match='strong' if ok else 'review'; trusted+=int(ok); reviewed+=int(not ok)
   if row.get('status')!=new_status or row.get('match_status')!=new_match or row.get('validation_version')!='2.3':
   ...
   else: row['validation_reason']=reason
...
def search_query(p,ts=None):
 ...
 # Rotate exact-family queries without increasing searches-per-product.
 d=datetime.fromisoformat((ts or now()).replace('Z','+00:00')).date().toordinal(); salt=int(hashlib.sha1(p['id'].encode()).hexdigest()[:8],16)
 return terms[(d+salt)%len(terms)]
