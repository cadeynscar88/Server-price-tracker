...
def match_result(product, result):
    title=norm(result.get('title')); pid=product.get('id','')
    ...
    if pid in {'nvme-4tb','nvme-2tb'}:
        cap='4tb' if pid=='nvme-4tb' else '2tb'; models=('sn850x','nm790','t500')
    ...
    model=norm(product.get('model')); manufacturer=norm(product.get('manufacturer'))
    ...
    if distinctive:
        ...
        if sum(t in title for t in set(distinctive)) < needed: return False,'distinctive model terms missing'
        return True,'manufacturer plus distinctive model terms'
    return False,'no unique model identifier available'

def cleanup_legacy():
    """Stop old pre-v1.3 SerpApi guesses from remaining trusted by the PWA."""
    changed=0
    for product in PRODUCTS:
        ...
        for row in rows:
            if row.get('method')=='serpapi_google_shopping' and row.get('status')=='verified' and not row.get('match_status'):
                ok,reason=match_result(product,{'title':row.get('model','')})
                row['match_status']='strong' if ok else 'review'
                row['notes']=(row.get('notes','')+' Legacy result revalidated: '+reason).strip()
...
def observation(product,slug,result,price,ts,status,reason):
    return {'component_id':product['id'],'component':product['label'],'model':result.get('title') or product.get('model',''),'retailer':slug,'price':price,'currency':'USD','source_url':source_url(result,(product.get('retailer_search_urls') or {}).get(slug,'')),'availability':'Shown in Google Shopping','status':status,'method':'serpapi_google_shopping','match_status':'strong' if status=='verified' else 'review','timestamp':ts,'notes':reason}

def collect_product(product,key,ts):
    query=(product.get('search_terms') or [product.get('model') or product.get('label')])[0]
    payload=serpapi_search(query,key); strong={}; review={}
    for result in payload.get('shopping_results') or []:
        ...
        ok,reason=match_result(product,result); bucket=strong if ok else review
    ...
    for slug in expected_retailers(product):
        if slug in strong: append(product['id'],strong[slug]); verified+=1
        elif slug in review: append(product['id'],review[slug]); reviews+=1
        else:
            append(product['id'],{'component_id':product['id'],'component':product['label'],'model':product.get('model',''),'retailer':slug,'source_url':(product.get('retailer_search_urls') or {}).get(slug,''),'availability':'Unknown','status':'not_found','method':'serpapi_google_shopping','timestamp':ts,'notes':'Retailer had no acceptable Shopping result for this item.'}); missing+=1
...
def main():
    ts=now(); legacy=cleanup_legacy(); key=os.getenv('SERPAPI_API_KEY','').strip()
    ...
    if not key:
        status={'checked_at':ts,'source':'serpapi_google_shopping','searches_attempted':0,'verified_retailer_offers':0,'manual_review_candidates':0,'missing_retailer_results':0,'check_failures':1,'legacy_results_revalidated':legacy,'note':'SERPAPI_API_KEY is not configured in GitHub Actions Secrets.'}
    ...
    status={'checked_at':ts,'source':'serpapi_google_shopping','searches_attempted':len(selected),'batch_size':batch_size,'verified_retailer_offers':verified,'manual_review_candidates':reviews,'missing_retailer_results':missing,'check_failures':failures,'legacy_results_revalidated':legacy,'note':'Only strong model matches are verified. Ambiguous offers require manual review and never affect build totals.'}
