...
self.addEventListener("fetch", e => {
 ...
 if(e.request.mode==="navigate"&&(url.pathname.endsWith("/")||url.pathname.endsWith("/index.html"))){
  e.respondWith(fetch(e.request).then(async res=>{let html=await res.text();if(!html.includes("history-hook.js"))html=html.replace("</body>","<script src=\"history-hook.js?v=1\"></script></body>");return new Response(html,{status:res.status,statusText:res.statusText,headers:{"content-type":"text/html; charset=utf-8"}})}).catch(()=>caches.match("./index.html")));return;
 }
